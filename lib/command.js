const { recordStreamFromArgs, s } = prequire("lib/streamUtils");
const JSONOutputStream = prequire("lib/jsonOutputStream.js");
...
const Command = (module.exports = class Command {
  ...
  jsonInputStream() {
    return (this._inputStream ??= recordStreamFromArgs({
      files: this.args,
    }));
  }

  jsonOutputStream() {
    return (this._outputStream ??= s(new JSONOutputStream()));
  }
});
